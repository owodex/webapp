...
class Transaction(models.Model):
    ...
    SERVICE_TYPES = (
        ...
        ('electricity', 'Electricity Bill'),
        ('bank transfer', 'Bank Transfer'),
        ('refund', 'Refund')  # Add this new service type
    )
    ...
    def update_wallet_on_cancellation(self):
        if self.status == 'cancelled' and self.transaction_type == 'debit':
            if self.wallet:
                ...
                Transaction.objects.create(
                    ...
                    service='refund',
                    status='completed',
                    invoice_id=f'RF-{self.invoice_id}'
                )
...
def transaction_pre_save(sender, instance, **kwargs):
    if instance.id:  # Check if this is an existing instance
        ...
        if old_instance.status != 'completed' and instance.status == 'completed':
            ...
            instance.update_wallet_on_cancellation()
